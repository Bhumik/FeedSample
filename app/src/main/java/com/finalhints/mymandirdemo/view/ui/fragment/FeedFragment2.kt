...
import com.finalhints.mymandirdemo.view.adapter.FeedListAdapter2
...
class FeedFragment2 : BaseFragment() {

    companion object {
        fun newInstance(): FeedFragment2 {
            val feedFragment = FeedFragment2()
            val sArgs = Bundle()
    ...
    private lateinit var mAdapter: FeedListAdapter2
    ...
    private fun mapUIelements(view: View) {
        ...
        mViewModel.emptyState.observe(this, Observer {
            when (it) {
                AppUtils.EmptyState.GONE -> emptyState.isVisible = false
    ...
    private fun initRecyclerView() {
        mAdapter = FeedListAdapter2(context!!, mFeedItems)
        rcvFeedList.apply {
            adapter = mAdapter
